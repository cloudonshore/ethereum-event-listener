...
class EventTracker {
  ...
  fetchHistoricalLogs(
    ...
    this.fetchLogs(contractAddress, abi, topics, fromBlock, toBlock, parser).then(events => {
  ...
  subscribeToEvent(event, blockNumber) {
    ...
    return this.subscribe(contract, abi, topics, blockNumber, callback, parser)
  }
  ...
  parseEventLogs(logs, abi) {
    ...
    return _.compact(logs.map(log => this.parseEventLog(log, abiInterface)))
  }
  fetchLatestBlock(includeFullTransactions = true) {
    // const method = {
    //   method: 'eth_getBlockByNumber',
    //   params: ['latest', includeFullTransactions], // [hex block number, include full transactions boolean]
    // }
    return this.web3Provider.eth.getBlock('latest', includeFullTransactions) // send(method).then(parseBlock)
  }
  getLogs([params]) {
    return this.web3Provider.eth.getPastLogs(params) // send(method)
  }
  getProviderLogs(params, provider) {
    const method = {
      method: 'eth_getLogs',
      params,
    }
  ...
  parseEventLog(log, abiInterface) {
    let parsedLog
    try {
  ...
  async fetchLogs(contractAddress, abi, topic, fromBlock, toBlock, parser, provider) {
    const query = {
    ...
    try {
      logs = await (provider ? this.getProviderLogs(logParams, provider) : this.getLogs(logParams))
    } catch (e) {
      console.log(`logs not ready for block ${toBlock}, retrying in 1s`, e, logParams)
      return new Promise((resolve, reject) => {
        setTimeout(
          () =>
            this.fetchLogs(contractAddress, abi, topic, fromBlock, toBlock, parser)
              .then(resolve)
              .catch(reject),
          1000,
        )
    ...
    const parsedEventLogs = this.parseEventLogs(logs, abi)
    return parser ? parser(parsedEventLogs) : parsedEventLogs
  }
  async subscribe(contractAddress, abi, topic, fromBlock, callback, parser) {
    const query = {
      address: contractAddress || undefined,
      topics: topic,
    }
    ...
    const subscription = this.web3Provider.eth.subscribe('logs', logParams, (error, log) => {
      if (error) {
        console.log(error)
      }
      if (parser) {
        callback(parser(this.parseEventLog(log, abiInterface)))
      } else {
        const parsedEventLog = this.parseEventLog(log, abiInterface)
